...
def clean_lyric_content(lyric_text):
    try:
        contributor_pattern = re.compile(r'\d+ ContributorsTranslations.*?Lyrics')
...
def lyrics_prompter(lyrics, genre, artist, year):
    prompted_lyric = f'[s:genre]{genre_mappings[genre]}[e:genre]' + ' ' + f'[s:artist]{artist}[e:artist]' + ' ' + f'[s:year]{year}[e:year]' + " " + f'[s:lyrics] {lyrics} [e:lyrics]' + '\n'
...
def process_lyrics_file(input_filepath, output_filepath, genres_artists):
    ...
    final_lyrics_df = pd.DataFrame(columns=['title', 'artist', 'tag', 'year', 'views', 'lyrics', "cleaned_lyrics", "final_lyrics"])
    
    for genre, artists in genres_artists.items():
        genre_df = df[df['tag'].str.lower() == genre.lower()]
        for artist in artists:
            artist_df = genre_df[genre_df['artist'].str.contains(artist, case=False, regex=False)]
            top_songs = artist_df.nlargest(min(300, len(artist_df)), 'views')
            
            # Clean lyrics
            top_songs['cleaned_lyrics'] = top_songs['lyrics'].apply(clean_lyric_content)
            top_songs = top_songs.dropna()
            
            # Prepare tags and combine
            top_songs['final_lyrics'] = top_songs.apply(lambda x: lyrics_prompter(x["cleaned_lyrics"], genre, artist, x["year"]), axis=1)

            final_lyrics_df = pd.concat([final_lyrics_df, top_songs], ignore_index=True)
...
if __name__ == "__main__":
    ...
    # File paths
    input_filepath = Path(os.path.join(PROJECT_DIR, "data", "processed", "lyrics_processed.csv"))
    output_filepath = Path(os.path.join(PROJECT_DIR, "data", "features", "lyrics_features.csv"))
